...
public class Sequence3<T> extends SequenceSecondary<T> {
    ...
    private static <T> void setLengthOfLeftStack(Stack<T> leftStack,
        ...
        if (leftStack.length() > newLeftLength) {
            //make a while loop that pushes values of the left stack onto
            //the right until it is equal to the new left length
            while (leftStack.length() != newLeftLength) {
                T x = leftStack.pop();
                rightStack.push(x);
            }
        } else {
            //make a while loop that pushes values of the right stack onto
            //the left until it is equal to the new left length
            while (leftStack.length() != newLeftLength) {
                T x = rightStack.pop();
                leftStack.push(x);
            }
